...
if __name__ == "__main__":
    ...
    if (request.ok): # if 200
        ...
        # output to file
        query_page_path = os.path.join(constants.DATASET_FOLDER, 'query_page.html')
        with open(query_page_path, 'w', encoding='utf-8') as file:
            print(parsed_content.prettify(), file=file)
    
    # find ordered list for most frequently downloaded in last 30 days header
    ID = "books-last30"
    ol_path = os.path.join(constants.DATASET_FOLDER, "ol.html")
    with open(ol_path, 'w', encoding='utf-8') as file:
        print(parsed_content.find(id=ID).find_next("ol"), file=file)
    
    # get list of ebook numbers and titles for downloading
    ebook_nums = []
    titles = []
    with open(ol_path, 'r', encoding='utf-8') as file:
    ...
    # remove special characters, numbers, author's name, spaces from titles, and add .txt
    for i in range(0, len(titles)):
        titles[i] = re.sub(r'[^a-zA-Z0-9\s]', '', titles[i])
        titles[i] = re.sub(r'\d+', '', titles[i]) 
        by_slice = titles[i].find(" by ")
        titles[i] = titles[i][:by_slice]
        titles[i] = titles[i].replace(' ', '_')
        titles[i] += '.txt'
    
    # download all books that can be downloaded as a txt file (TLP is usually in the list :/)
    SAVE_PATH = constants.WORKS_FOLDER
    for i, num in enumerate(ebook_nums):
        if (titles[i] == ".txt"): # avoids adding some non-English titles to list
            continue
