...
if __name__ == "__main__":
    ...
    print(f"\nCurrently preprocessing {f_title}")
    with open(arg, 'r', encoding="utf8") as file:
        ...
        main_dir = constants.DOCUMENTS_FOLDER
        document_folder = os.path.join(main_dir, f_title + '_docs')
        
        if not os.path.exists(document_folder):
            os.makedirs(document_folder)
        
        for i, doc in enumerate(documents):
           doc_fname = os.path.join(document_folder, f_title + "_doc_" + str(i + 1) + '.txt') 
